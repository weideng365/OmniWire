...
OmniWire 是一个网络安全网关系统，提供 WireGuard / OpenVPN 服务器管理、TCP/UDP 端口转发和端口监控功能，通过 Web 界面进行操作。
...
8. **前端打包后必须同步到 packed 目录**：前端构建输出到 `server/resource/public`，但 embed 嵌入的是 `server/internal/packed/public`。同步前必须**先清空** `internal/packed/public`，再完整复制 `resource/public` 内容，否则会残留历史 hash 文件导致二进制膨胀。`build.sh` / `build.bat` / `Dockerfile` 已统一处理。
9. **新增页面必须同步修改 MainLayout.vue 菜单**：在 `router/index.js` 添加路由后，必须同时在 `layouts/MainLayout.vue` 的 `el-menu` 中添加对应菜单项，否则侧边栏不会显示新页面入口。
10. **静态资源嵌入只用 Go 原生 `embed`，不要用 `gf pack`**：项目曾使用 `gf pack` 生成 `internal/packed/packed.go`（基于 `gres`），但 `cmd.go` 实际只读 `embed.FS`，导致 `gf pack` 产物完全无人使用且与 `embed` 嵌入内容脱节。已彻底移除 `packed.go` 与 `gf pack` 步骤。如需嵌入资源，编译时加 `-tags embed`。
11. **OpenVPN 回调端点必须限制本地源**：`/api/v1/openvpn/auth`、`/connect`、`/disconnect` 由 OpenVPN 守护进程在本机执行 up/down 脚本回调，必须校验 `RemoteAddr` 为 loopback；放任白名单将允许外网伪造身份认证。
...
- `internal/service/` — 业务逻辑：
  ...
  - `openvpn/` — OpenVPN 服务管理与用户认证（通过 management/up-down 脚本回调本机 API）
...
**路由**：所有 API 路由在 `/api/v1` 下分组，使用 `ghttp.MiddlewareHandlerResponse` 中间件统一 JSON 响应格式。子分组：`/system`、`/wireguard`、`/openvpn`、`/forward`、`/port`。OpenVPN 回调路径（`/openvpn/auth|connect|disconnect`）放行 JWT 但限制 loopback 源。
...
- WireGuard：接口默认 `omniwire`（可在 `wireguard.interface` 配置覆盖），端口 `51820/udp`，子网 `10.66.66.0/24`
- OpenVPN：端口 `1194/udp`（可在 `openvpn.port` 配置覆盖）
- 默认凭据：`admin` / `admin123`
- 默认启用 JWT 认证
...
1. `npm run build` → 输出到 `server/resource/public/`
2. 构建脚本清空 `server/internal/packed/public/`，再完整复制 `resource/public/` 内容到该目录
3. `go build -tags embed` → `internal/packed/embed.go` 生效，`//go:embed all:public` 嵌入静态资源
4. 不加 `-tags embed` 时使用 `embed_stub.go`，运行时回退到本地 `resource/public` 目录

生产部署始终使用 `-tags embed`（参见 `Dockerfile` 与 `build.sh` / `build.bat`）。
